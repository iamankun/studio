...
export default function UploadFormView({
  ...
  const [isCopyrightOwner, setIsCopyrightOwner] =
    useState<CopyrightOwnershipStatus>("yes");
  const [hasBeenReleased, setHasBeenReleased] =
    useState<ReleaseHistoryStatus>("no");
    ...
    return (
      <div className="p-6 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">Giai điệu đang vang lên...</p>
        </div>
      </div>
    );
  }
      ...
      prev.map((track) => {
        if (track.id === trackId) {
          return {
            ...
            info: {
              ...track.info,
              additionalArtists: track.info.additionalArtists.filter(
      ...
      prev.map((track) => {
        if (track.id === trackId) {
          return {
            ...
            info: {
              ...track.info,
              additionalArtists: track.info.additionalArtists.map(
                (artist, index) => {
  ...
  const handleSubmit = async () => {
    ...
    const invalidTracks = audioTracks.filter(
      (track) => !track.info.songTitle.trim()
    );
    if (invalidTracks.length > 0) {
      validationErrors.push(
        `${invalidTracks.length} track chưa có tên bài hát`
      );
    }
    ...
    audioTracks.forEach((track) => {
      track.info.additionalArtists.forEach((artist, index) => {
        if (!artist.name.trim()) {
          validationErrors.push(
            `Track "${track.info.songTitle}": Nghệ sĩ phối hợp #${index + 1} chưa có tên`
          );
        }
      });
    });
    ...
    try {
      ...
      const tracksData: Omit<
        PrismaTrack,
        "id" | "createdAt" | "updatedAt" | "submissionId"
      >[] = [];
      ...
      for (const track of audioTracks) {
        ...
        if (!audioResult.success) {
          throw new Error(
            `Lỗi upload âm thanh ${track.file.name}: ${audioResult.message}`
          );
        }
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-2">
              🎵 Upload nhạc để phát hành toàn cầu
            </h1>
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      value={artistRole}
                      onValueChange={(value: ArtistPrimaryRole) =>
                        setArtistRole(value)
                      }
                    >
                      <SelectTrigger>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div>
                  <Label htmlFor="imageUpload">
                    Ảnh bìa bài hát (JPG, 4000x4000px, tối đa 10MB) *
                  </Label>
                ...
                <div>
                  <Label htmlFor="audioUpload">
                    File nhạc (WAV 24-bit, 2 kênh, 192kHz, tối đa 100MB mỗi
                    file) *
                  </Label>
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                    <input
                        ...
                        {audioTracks.map((track, index) => (
                          <div key={track.id} className="border rounded-lg p-4">
                              ...
                              <div>
                                <Label htmlFor={`track-isrc-${track.id}`}>
                                  ...
                                  <span className="text-sm text-gray-500 ml-1">
                                    - Để trống nếu chưa có, sẽ tự tạo khi
                                    release
                                  </span>
                                </Label>
                                <Input
                            ...
                            <div className="mt-4">
                              ...
                              {track.info.additionalArtists.map(
                                (artist, artistIndex) => (
                                  <div
                                    key={artistIndex}
                                    className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-2 p-2 bg-gray-50 rounded"
                                  >
                                    <Input
                                    ...
                                    <Input
                                      ...
                                      value={
                                        artist.fullName || "Theo giấy tờ CCCD"
                                      }
                                      onChange={(e) =>
                                        updateAdditionalArtist(
                                          track.id,
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      value={releaseType}
                      onValueChange={(value: ReleaseType) =>
                ...
                <div>
                  <Label>
                    Bạn có sở hữu bản quyền của bài hát này không? *
                  </Label>
                  <RadioGroup
                    value={isCopyrightOwner}
                    onValueChange={(value: CopyrightOwnershipStatus) =>
                      setIsCopyrightOwner(value)
                    }
                ...
                <div>
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value: ReleaseHistoryStatus) =>
                      setHasBeenReleased(value)
                    }
                    className="flex gap-4 mt-2"
                  >
                    <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-2">
                      {[
                        ...
                        {
                          value: "apple_music" as Platform,
                          label: "Apple Music",
                        },
                        {
                          value: "soundcloud" as Platform,
                          label: "SoundCloud",
                        },
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Ngày phát hành phải ít nhất 2 ngày kể từ hôm nay hoặc đã
                    từng phát hành thì nhập ngày cũ
                  </p>
                </div>
                ...
                <div className="bg-secondary/50 border border-border/50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm text-foreground">
                    <p>
                      <strong>Nghệ sĩ:</strong> {artistName || "Chưa nhập"}
                    </p>
                    <p>
                      <strong>Bài hát:</strong> {songTitle || "Chưa nhập"}
                    </p>
                    <p>
                      <strong>Album:</strong> {albumName || "Không có"}
                    </p>
                    <p>
                      <strong>Thể loại:</strong> {mainCategory} - {subCategory}
                    </p>
                    <p>
                      <strong>Loại phát hành:</strong> {releaseType}
                    </p>
                    <p>
                      <strong>Files âm thanh:</strong> {audioTracks.length}{" "}
                      file(s)
                    </p>
                    <p>
                      <strong>Có ảnh bìa:</strong>{" "}
                      {imageFile ? "Có" : "Chưa có"}
                    </p>
                    <p>
                      <strong>Ngày phát hành:</strong>{" "}
                      {releaseDate || "Chưa chọn"}
                    </p>
                  </div>
                </div>
