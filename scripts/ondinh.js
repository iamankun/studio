...
async function DatabaseSchema(client) {
    const requiredTables = ['User', 'Label_Manager', 'Submission', 'nhat_ky_studio', 'label_templates','label_assignments'];
    let tableCount = 0;
...
async function AuthorizationLogic(client) {
    try {
        ...
        if (label_manager.rows.length > 0 && label_manager.rows[0].label_id && label_manager.rows[0].artist_id) {
            console.log('✅  Quản lí nhãn phát hành có - Chấp nhận     (La: Chap nhận)');
            return true;
        } else {
